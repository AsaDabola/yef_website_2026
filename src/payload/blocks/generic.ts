...
/* --------------------------------------------------------------------------
 * The Who We Are page shapes.
 *
 * These three carry the design of the Welcome and Our Mission pages rather
 * than a generic approximation of it. That distinction is the whole point:
 * those pages were once converted to genericText/genericQuote and the words
 * survived while the design did not — the lead paragraph, the photo card and
 * the two-column split have no generic equivalent, so they were flattened
 * into stacked paragraphs.
 *
 * Each renders through the same components and the same classes the pages
 * use, so an editor can change every word and picture without being able to
 * change the design out from under itself.
 * ----------------------------------------------------------------------- */

export const GenericLeadBlock: Block = {
  slug: "genericLead",
  labels: { singular: "Lead paragraph", plural: "Lead paragraphs" },
  ...
  fields: [
    ...
    { name: "body", type: "textarea", required: true },
    {
      ...
      defaultValue: "large",
      options: [
        { label: "Large (Welcome)", value: "large" },
        { label: "Medium (Our Mission)", value: "medium" },
      ],
      admin: { description: "How big the paragraph is set." },
    },
  ],
};

export const GenericFeatureCardBlock: Block = {
  slug: "genericFeatureCard",
  labels: { singular: "Text beside photo card", plural: "Text beside photo cards" },
  admin: { group: "Page sections" },
  fields: [
    backgroundField,
    {
      ...
      type: "array",
      minRows: 1,
      admin: { description: "The body copy beside the card, in this order." },
      fields: [{ name: "body", type: "textarea", required: true }],
    },
    { name: "image", type: "upload", relationTo: "media", required: true },
    { name: "imageAlt", type: "text" },
    {
      type: "row",
      fields: [
        { name: "cardEyebrow", type: "text", required: true, admin: { width: "50%" } },
        {
          name: "cardEyebrowLine2",
          type: "text",
          admin: { width: "50%", description: "Optional second line, e.g. a verse reference." },
        },
      ],
    },
    { name: "cardTitle", type: "text", required: true },
    {
      ...
      defaultValue: "right",
      options: [
        { label: "Card on the right", value: "right" },
        { label: "Card on the left", value: "left" },
      ],
    },
  ],
};

export const GenericSplitColumnsBlock: Block = {
  slug: "genericSplitColumns",
  ...
  fields: [
    ...
    { name: "heading", type: "text" },
    {
      name: "quote",
      type: "textarea",
      admin: { description: "Shown under the heading, in brand blue." },
    },
    {
      name: "introBold",
      type: "textarea",
      admin: { description: "The bold line that introduces the list." },
    },
    {
      name: "items",
      type: "array",
      admin: { description: "The list under the bold line, in brand blue." },
      fields: [{ name: "body", type: "text", required: true }],
    },
    {
      name: "paragraphs",
      type: "array",
      admin: { description: "The right-hand column." },
      fields: [
        { name: "body", type: "textarea", required: true },
        {
          name: "boldSuffix",
          type: "textarea",
          admin: { description: "Optional bold sentence closing this paragraph." },
        },
      ],
    },
  ],
...
export const genericBlocks: Block[] = [
  ...
  GenericLeadBlock,
  GenericFeatureCardBlock,
  GenericSplitColumnsBlock,
];
